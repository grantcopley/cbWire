...
export default {
    input: 'resources/assets/js/livewire/index.js',
    output: {
        format: 'umd',
        sourcemap: true,
        name: 'Livewire',
        file: 'includes/js/livewire.js',
    },
    plugins: [
        resolve(),
        commonjs({
        ...
        alias({
            entries: [
                { find: '@', replacement: __dirname + '/resources/assets/js/livewire' },
            ]
        }),
        // Mimic Laravel Mix's mix-manifest file for auto-cache-busting.
        outputManifest({
            serialize() {
                const file = fs.readFileSync(__dirname + '/includes/js/livewire.js', 'utf8');
